...
from models.dynamicEllipsoidObstaclesRNN_customPooling import StateEncoder, closest_pooling
...
class CommonEncoder(tfkl.Layer):
    def __init__(self, args):
        ...
        self.lstm_other_quads_pos = tfkl.LSTM(self.rnn_state_size_lstm_other_quads//2, name = 'lstm_other_quads_pos', return_sequences = False, kernel_regularizer = tfkr.l2(self.reg_factor), recurrent_regularizer = tfkr.l2(self.reg_factor), bias_regularizer = tfkr.l2(self.reg_factor), activity_regularizer = tfkr.l2(self.reg_factor))
        self.lstm_other_quads_vel = tfkl.LSTM(self.rnn_state_size_lstm_other_quads//2, name = 'lstm_other_quads_vel', return_sequences = False, kernel_regularizer = tfkr.l2(self.reg_factor), recurrent_regularizer = tfkr.l2(self.reg_factor), bias_regularizer = tfkr.l2(self.reg_factor), activity_regularizer = tfkr.l2(self.reg_factor))
        
        self.fc_obs_pos = tfkl.Dense(self.fc_state_size_obstacles//4, activation = 'relu')
        self.fc_obs_vel = tfkl.Dense(self.fc_state_size_obstacles//4, activation = 'relu')
    ...
    def call(self, x):        
        ...
        for quad_idx in range(x["others_input"].shape[-1]):
            processed_positions = self.lstm_other_quads_pos(x["others_input"][:, :, 0:3, quad_idx])
            processed_velocities = self.lstm_other_quads_vel(x["others_input"][:, :, 3:6, quad_idx])
            
            processed_objects.append(self.concat([processed_positions, processed_velocities]))
        
        closest_3_points = closest_pooling(x["obstacles_input"][:, 0:3, :, :])
        for obs_idx in range(x["obstacles_input"].shape[-2]):
            ...
            processed_objects.append(self.concat([processed_positions1, processed_positions2, processed_positions3, processed_velocities]))
