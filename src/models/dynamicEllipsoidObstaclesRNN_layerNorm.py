...
class StateEncoder(tfkl.Layer):
    def __init__(self, rnn_state_size = 256, reg_factor = 0.01):
        super().__init__()
        ...
        self.lstm_quad = tfkl.LSTM(self.rnn_state_size, name = 'lstm_state', return_sequences = False,\
            kernel_regularizer = tfkr.l2(self.reg_factor), recurrent_regularizer = tfkr.l2(self.reg_factor), bias_regularizer = tfkr.l2(self.reg_factor), activity_regularizer = tfkr.l2(self.reg_factor))
...
class OtherQuadsEncoder(tfkl.Layer):
    def __init__(self, rnn_state_size_lstm_quad = 256, rnn_state_size_bilstm = 256, reg_factor = 0.01):
        super().__init__()
        ...
        self.bilstm = tfkl.Bidirectional(\
            tfkl.LSTM(self.rnn_state_size_bilstm,  name = 'bilstm_other_quads', return_sequences = False, return_state = False,\
                kernel_regularizer = tfkr.l2(self.reg_factor), recurrent_regularizer = tfkr.l2(self.reg_factor), bias_regularizer = tfkr.l2(self.reg_factor), activity_regularizer = tfkr.l2(self.reg_factor)), merge_mode = 'ave') # Average of forward and backward LSTMs
        self.layer_norm1 = tfkl.LayerNormalization()
        self.layer_norm2 = tfkl.LayerNormalization()
    ...
    def call(self, x):
        ...
        for quad_idx in range(x.shape[-1]):
            aux = self.lstm_other_quads(x[:, :, :, quad_idx])
            aux = self.layer_norm1(aux)
            other_quads.append(aux)
...
class DynamicObstaclesEncoder(tfkl.Layer):
    def __init__(self, fc_units = 64, rnn_state_size_bilstm = 64, reg_factor = 0.01):
        super().__init__()
        ...
        self.fc_obs = tfkl.Dense(self.fc_units, activation = 'relu',\
            kernel_regularizer = tfkr.l2(self.reg_factor), bias_regularizer = tfkr.l2(self.reg_factor), activity_regularizer = tfkr.l2(self.reg_factor))
        self.bilstm = tfkl.Bidirectional(\
...
class Decoder(tfkl.Layer):
    def __init__(self, rnn_state_size_lstm_concat = 512, fc_hidden_unit_size = 256, reg_factor = 0.01):
        super().__init__()
        ...
        self.lstm_concat = tfkl.LSTM(self.rnn_state_size_lstm_concat, name = 'lstm_decoder', return_sequences = True, return_state = False,\
            kernel_regularizer = tfkr.l2(self.reg_factor), recurrent_regularizer = tfkr.l2(self.reg_factor), bias_regularizer = tfkr.l2(self.reg_factor), activity_regularizer = tfkr.l2(self.reg_factor))
        self.layer_norm = tfkl.LayerNormalization()
        # self.lstm_concat = tfkl.Bidirectional(tfkl.LSTM(self.rnn_state_size_lstm_concat, name = 'lstm_decoder', return_sequences = True, return_state = False), merge_mode = 'ave')
