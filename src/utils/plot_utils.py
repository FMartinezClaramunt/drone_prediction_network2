...
from utils.data_handler import prepare_data, scale_data, unscale_output
...
class plot_scenario(object): 
    def __init__(self, model, args, save = False, display = True, figsize = (8,6), nframes = 10000, dt = 0.05): # 0.05 is the sampling period for the CCMPC_MAV simulator
        ...
        self.past_trajectories, __ = prepare_data(test_data_paths, self.past_steps, self.future_steps, X_type = 'pos', Y_type = 'pos', split = False, shuffle = False)
        
        X_test, Y_test = prepare_data(test_data_paths, self.past_steps, self.future_steps, X_type = X_type, Y_type = Y_type, split = split, shuffle = False)
        ...
        if args['scale_data']:
            ...
            # Y_test = scale_data(Y_test, output_scaler)
        ...
        if split == False and Y_type == 'pos': # Absolute coordinates
            self.pred_trajectories = Y_predicted
            self.future_trajectories = Y_test
        elif split == True and Y_type == 'pos': # Relative coordinates
            self.pred_trajectories = Y_predicted + self.past_trajectories[:, -1:, 0:3] # Relative to absolute coordinates
            self.future_trajectories = Y_test + self.past_trajectories[:, -1:, 0:3]
        elif split == True and Y_type == 'vel':
        ...
        elif split == False and Y_type == 'vel':
            self.pred_trajectories = np.zeros([Y_predicted.shape[0], Y_predicted.shape[1]+1, Y_predicted.shape[2]])
            self.pred_trajectories[:, 0, :] = self.past_trajectories[:, -1, :]
            self.future_trajectories = np.zeros([Y_predicted.shape[0], Y_predicted.shape[1]+1, Y_predicted.shape[2]])
            self.future_trajectories[:, 0, :] = self.past_trajectories[:, -1, :]
            ...
            raise Exception("Unavailable plotting configuration")
