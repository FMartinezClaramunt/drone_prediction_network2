...
class plot_predictions():
    def __init__(self, data, args, recording_path, figsize=(8,6)):
        ...
        """
        self.nframes = data['predictions'].shape[0]
        self.n_total_quadrotors = data['trajectories'].shape[2]
        if data['predictions'].ndims == 3:
    ...
    def setup_plot(self):
        ...
        if self.goals == None:
            ...
            goals = [ self.ax.plot3D([], [], [], 'o', color = 'c')[0] for i in range(self.n_total_quadrotors) ]
    ...
    def animate(self, iteration):
        ...
        for quad in range(self.n_predicted_quadrotors):
            self.plot_objects['future_trajectories'][quad].set_data( self.trajectories[current_idx:future_idx, 0, quad], self.trajectories[current_idx:future_idx, 1, quad] )
            self.plot_objects['future_trajectories'][quad].set_3d_properties( self.trajectories[current_idx:future_idx, 2, quad] )
